<x-app-layout>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Logika untuk Induk -> Anak
            document.querySelectorAll('.group-checkbox').forEach(function (groupCheckbox) {
                groupCheckbox.addEventListener('change', function () {
                    const groupSlug = this.dataset.group;
                    const isChecked = this.checked;
                    document.querySelectorAll('.feature-checkbox.' + groupSlug).forEach(function (featureCheckbox) {
                        featureCheckbox.checked = isChecked;
                    });
                });
            });

            // LOGIKA BARU: untuk Anak -> Induk
            document.querySelectorAll('.feature-checkbox').forEach(function (featureCheckbox) {
                featureCheckbox.addEventListener('change', function () {
                    const groupSlug = this.classList[1]; // Mengambil slug grup dari kelas kedua
