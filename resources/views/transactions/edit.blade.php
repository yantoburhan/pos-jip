<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Ubah Transaksi: ') }} {{ $transaction->no_transaksi }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('transactions.update', $transaction->no_transaksi) }}" id="transaction-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                            <div>
                                ...
                                <input id="date" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="date" name="date" value="{{ old('date', \Carbon\Carbon::parse($transaction->date)->format('Y-m-d')) }}" required />
                            </div>

                            <div class="relative">
                                <label for="customer_search" class="block font-medium text-sm text-gray-700">{{ __('No. HP Customer') }}</label>
                                <input id="customer_search" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="text" placeholder="Ketik No. HP atau Nama Customer..." autocomplete="off" value="{{ old('no_hp_cust', $transaction->no_hp_cust) }}" />
                                <input type="hidden" name="no_hp_cust" id="no_hp_cust" value="{{ old('no_hp_cust', $transaction->no_hp_cust) }}">
                                <div id="customer_suggestions" class="absolute z-20 w-full bg-white border rounded-md mt-1 shadow-lg hidden"></div>
                                <div id="customer-info-display" class="mt-2">
                                    {{-- Tampilkan info customer yang ada saat ini --}}
                                    @if($transaction->customer)
                                        <div class="p-2 bg-green-50 border border-green-200 rounded-md text-sm text-green-700">Customer: <strong>{{ $transaction->customer->cust_name }}</strong></div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="alamat" name="alamat" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('alamat', $transaction->alamat) }}</textarea>
                            </div>
                             ...
                             <div>
                                ...
                                <select name="wilayah" id="wilayah" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    <option value="tidak_diketahui" @if(old('wilayah', $transaction->wilayah) == 'tidak_diketahui') selected @endif>Tidak Diketahui</option>
                                    <option value="medan" @if(old('wilayah', $transaction->wilayah) == 'medan') selected @endif>Medan</option>
                                    <option value="luar_medan" @if(old('wilayah', $transaction->wilayah) == 'luar_medan') selected @endif>Luar Medan</option>
                                </select>
                            </div>
                            
                            <div>
                                <label for="kecamatan" class="block font-medium text-sm text-gray-700">{{ __('Kecamatan (Opsional)') }}</label>
                                <input id="kecamatan" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" type="text" name="kecamatan" value="{{ old('kecamatan', $transaction->kecamatan) }}" />
                            </div>
                        </div>
                        ...
                        <div class="mt-10">
                            ...
                            <div id="transaction-items-container" class="space-y-4">
                                {{-- Loop melalui item yang ada (baik dari old input atau dari database) --}}
                                @php
                                    $items = old('items', $transaction->items->map(function ($item) {
                                        return [
                            ...
                            <template id="transaction-item-template">
                                {{-- Template ini sama persis dengan yang di halaman create --}}
                                <div class="grid grid-cols-12 gap-3 p-3 bg-gray-50 rounded-lg border items-center transaction-item-row">
                                    <div class="col-span-12 md:col-span-4 relative">
                        ...
                        <!-- Total -->
                         <div class="mt-10 pt-6 border-t-2 border-dashed">
                            {{-- ... (Bagian total sama seperti di create) ... --}}
...
<script>
// Script ini 99% sama dengan yang ada di create.blade.php
// Perbedaan utamanya adalah inisialisasi 'itemIndex' dan pemanggilan 'updateCalculations()' di awal
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Inisialisasi index berdasarkan item yang sudah ada
    ...
    // --- AJAX SEARCH (Sama seperti di create.blade.php) ---
    ...
    // Hitung total untuk data yang sudah ada saat halaman dimuat
    updateCalculations();
});
</script>
</x-app-layout>